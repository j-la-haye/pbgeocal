...
class PushbroomCamera:
    ...
    def __init__(self, angle_lut_path: str, smile: SmileModel,
        ...
        # ---- FOV angular bounds (for validity checking) ----
        # These are the hard limits of the angle LUT.  Any across-track
        # angle outside [theta_min, theta_max] is beyond the sensor FOV.
        # np.interp silently clamps to edge values, so we need explicit
        # bounds checking to reject points outside the FOV.
    ...
    def is_within_fov(self, P_cam: np.ndarray) -> np.ndarray:
        """
        Check if camera-frame direction is within the sensor field of view.

        This is the definitive across-track validity check.  np.interp in
        project() silently clamps out-of-range angles to the edge pixel,
        making is_valid_pixel() useless for rejecting off-FOV points.
        This method checks the actual across-track angle against the LUT
        angular range.

        Parameters
        ----------
        P_cam : (..., 3) points in camera frame [X-right, Y-back, Z-down]

        Returns
        -------
        within : (...,) boolean, True if across-track angle is within LUT range
        """
        Xc = P_cam[..., 0]
        Zc = P_cam[..., 2]
        ...
        # Lab-corrected across-track angle (same transform as project())
        x_lab = s * (Xc / Zc) + dcx
        theta_lab = np.arctan(x_lab)

        return ((theta_lab >= self.theta_min_rad) &
                (theta_lab <= self.theta_max_rad))
