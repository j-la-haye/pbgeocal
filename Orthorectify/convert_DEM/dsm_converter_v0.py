...
class PyProjTransformer:
    ...
    def _setup_height_transformer(self, config: Config):
        ...
        src_compound = CompoundCRS(
        name="LV95 + LN02",
        components=[CRS.from_epsg(2056), CRS.from_epsg(5728)]
    )
        try:
            ...
            logger.info(f"Using compound CRS transformation: {source_compound.name}")
            logger.info(f"  Source: EPSG:2056+5728 (LV95 + LN02 height)")
            logger.info(f"  Target: {self.target_crs.name}")
        except Exception as e:
            logger.warning(f"Could not create compound CRS transformer: {e}")
            logger.warning("Falling back to horizontal-only transformation")
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 1:
        sys.argv.append('Orthorectify/convert_DEM/config.yaml')
