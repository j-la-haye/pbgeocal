...
class TimeSolver:
    ...
    def solve(self, ground_ecef: np.ndarray,
        ...
        # Along-track residual: d_cam[:, 1] should be ~0 for valid pushbroom projection
        # (assuming y is the along-track axis in camera frame)
        along_track_residual = np.abs(d_cam[:, 0])

        # Scale threshold relative to range to handle varying altitude
        range_to_point = np.linalg.norm(d_cam, axis=1)
        normalized_residual = along_track_residual / np.maximum(range_to_point, 1e-6)

        # Threshold: e.g., IFOV-scale tolerance
        max_residual = 1e-4  # tune this — start loose and tighten

        valid = (converged &
                self.cam.is_valid_pixel(u_pixel) &
                (t >= self.t_min) & (t <= self.t_max) &
                (d_cam[:, 2] > 0) &
                (u_pixel >= 0) & (u_pixel < self.cam.num_pixels) &
                (y_lab >= 0) & (y_lab < len(self.exposure_times)) 
                #& (normalized_residual < max_residual)  # <-- this kills the curtains
                )
