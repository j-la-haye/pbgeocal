...
def process_tile(
    ...
    # At solved time t, get camera position
    cam_pos = trajectory.pos_ecef(t_init)  # [X, Y, Z] in ground frame

    # Horizontal distance from camera nadir to DSM cell
    dx = ground_ecef[:, 0] - cam_pos[:, 0]
    dy = ground_ecef[:, 1] - cam_pos[:, 1]
    ground_dist = np.sqrt(dx**2 + dy**2)

    # Max possible swath half-width from altitude and camera FOV
    altitude = cam_pos[:, 2] - ground_ecef[:, 2]
    half_fov = np.arctan(camera.num_pixels * camera.pixel_size / (2 * camera.focal_length))
    max_half_swath = altitude * np.tan(half_fov)

    valid =(ground_dist < max_half_swath * 1.2)  # small buffer
