...
def _worker_init(config_dict: dict):
    ...
    # Camera — decoupled model with LUT + smile + optics correction
    from .camera_model import SmileModel, OpticsCorrection
    smile = SmileModel(
        a=config.camera.smile.a,
        b=config.camera.smile.b,
        c=config.camera.smile.c,
    )
    correction = OpticsCorrection(
        f_lab=config.camera.optics.f_lab,
        f=config.camera.optics.f,
        cx=config.camera.optics.cx,
        cy=config.camera.optics.cy,
    )
    camera = PushbroomCamera(config.camera.angle_lut_path, smile, correction)

    # DSM
    dsm = DSMHandler(config.paths.dsm, config.crs.dsm_epsg)
