...
def process_tie_points(root_directory, output_bingo='bingo_output.txt', output_timing='timing_info.csv', 
    ...
    # filter all_observations_sorted to remove items where the landmark_id appears only once
    ...
    # Filter observations to keep only those with landmark_id appearing more than once
    all_observations_sorted = [obs for obs in all_observations_sorted if observation_counts[obs['landmark_id']] > 1]
