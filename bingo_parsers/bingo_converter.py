...
def process_tie_points(root_directory, output_bingo='bingo_output.txt', output_timing='timing_info.csv', 
                      timing_file_pattern='*timing*.csv',time_scale=1.0):
    """
    ...
    #sort landmarks by ascending time
    #for landmark_name in landmarks:
    #    landmarks[landmark_name].sort(key=lambda x: x['time'])
    ...
    # Write BINGO format output file sorted by time of each key in landmarks
    with open(output_bingo, 'w') as bingo_file:
        for landmark_name in sorted(landmarks[landmark_name], key=lambda x: x['time']).keys():
