...
def read_envi_rgb(hdr_file):
    ...
    # BIL format: (lines, bands, samples)
    # Need to transpose to (lines, samples, bands) = (height, width, channels)
    if len(data.shape) == 3 and img.shape[2] > 100:
        # For BIL: (lines, bands, samples) -> (lines, samples, bands)
        rgb = np.transpose(data, (0, 2, 1))
        print(f"After transpose: {rgb.shape}")   
    else:
        rgb = data
...
def load_matches(csv_file):
    ...
    """
    matches = pd.read_csv(csv_file, header=None,delimiter=' ',index_col=False)
    return matches
...
def visualize_matches(img1_hdr, img2_hdr, matches_csv, max_matches=None):
    ...
    # Plot matching points with numbered labels
    for idx, (_, match) in enumerate(matches.iterrows()):
        ...
        # Add numbered labels
        ax1.text(u1, v1, str(match_num), color='yellow', fontsize=10, fontweight='bold',
                ha='center', va='center',
                bbox=dict(boxstyle='circle,pad=0.1', facecolor='black', edgecolor='white', alpha=0.7))
        ax2.text(u2, v2, str(match_num), color='yellow', fontsize=10, fontweight='bold',
                ha='center', va='center',
                bbox=dict(boxstyle='circle,pad=0.1', facecolor='black', edgecolor='white', alpha=0.7))
...
def main():
    ...
    max_matches = 100  # Set to None to show all matches
