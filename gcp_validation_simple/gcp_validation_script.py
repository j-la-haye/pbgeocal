...
def geodetic_to_ecef(lat: float, lon: float, height: float) -> np.ndarray:
    """
...
def transform_ecef_to_camera(
    ...
    Args:
        point_ecef: Point coordinates in ECEF [X, Y, Z]
    ...
    Returns:
        Point coordinates in camera frame [X, Y, Z]
    """
...
def project_to_image(
    ...
    # Apply intrinsics
    u = camera.fx * x_d + camera.cx
    v = camera.fy * y_d + camera.cy
...
def run_validation(config_path: str, verbose: bool = True):
    ...
    camera = CameraIntrinsics(
        ...
        image_width=cam_cfg.get('image_width', 0),
        image_height=cam_cfg.get('image_height', 0),
    )
