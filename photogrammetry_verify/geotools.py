...
class GeoConverter:
    def __init__(self, epsg_code):
        # Transformer for Input CRS -> ECEF (EPSG:4978)
        self.to_ecef_transformer = pyproj.Transformer.from_crs(
            f"EPSG:{epsg_code}", "EPSG:4978", always_xy=True
        )
        # Transformer for Input CRS -> WGS84 Geodetic (for Lat/Lon calculation)
        self.to_wgs84_transformer = pyproj.Transformer.from_crs(
            f"EPSG:{epsg_code}", "EPSG:4326", always_xy=True
        )

    def to_ecef(self, coords):
        """Converts input coordinates [x, y, z] to ECEF [X, Y, Z]."""
        x, y, z = self.to_ecef_transformer.transform(coords[:, 0], coords[:, 1], coords[:, 2])
        return np.stack([x, y, z], axis=1)

    def get_enu_rotation_matrix(self, lon, lat):
        """
    ...
    def get_local_enu_points(self, points_ecef, origin_ecef, origin_lonlat):
        """
        Transforms ECEF points into a rigorous local ENU frame centered at origin.
        """
        lon, lat = origin_lonlat[0], origin_lonlat[1]
        R_ecef_enu = self.get_enu_rotation_matrix(lon, lat)
        
        # 1. Translate
        delta_ecef = points_ecef - origin_ecef
        
        # 2. Rotate: P_enu = R_ecef_enu * (P_ecef - Origin_ecef)
        points_enu = (R_ecef_enu @ delta_ecef.T).T
        return points_enu
...
class TangentPlane:
    def __init__(self, lat, lon, alt=0):
        ...
        self.xyz0 = np.array(lla2ecefTransformer.transform(self.lon,self.lat,alt, radians=True)).reshape(1,3)
...
class Trajectory:
    def __init__(self, t, lla, rpy, tp, t_span=None,radians=True):
        ...
        for i in range(len(t)):
            R_ned2body = R_ned2b(rpy[i,0], rpy[i,1], rpy[i,2])
            R_ned2ecef = R_ned2e(lla[i,0], lla[i,1])
            R_b2enu = tp.R_ecef2enu @ R_ned2ecef @ R_ned2body.T
            self.q_b2m[i,:] = dcm2quat(R_b2enu)
    ...
    def interpolate(self, timestamps, customRPY = True):
        ...
        poses = []
        for i in range(len(timestamps)):
            poses.append(Pose_std(timestamps[i], lla_interp[i,:], xyz_interp[i,:], rpy_interp[i,:],self.ecef[i,:], R_ned2e_interp[i], R_ned2b_interp[i].as_matrix()))
